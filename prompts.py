...
IMPORTANT: You MUST use the 'save_to_json' tool to save your results to {CRITIC_COMPANIES_JSON_PATH}. Even if NO companies match the requirements, you MUST still save one company from the original list to {CRITIC_COMPANIES_JSON_PATH} to avoid errors in the next steps as:

`save_to_json(<data>, path={CRITIC_COMPANIES_JSON_PATH})` where <data> is the new JSON object containing the companies that match the client's/strategy's requirements.
"""
